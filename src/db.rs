...
pub async fn check_item(client: &Client, list_id: i32, item_id: i32) -> Result<(), io::Error> {
    let statement = client.prepare("update todo_item set checked = true where list_id = $1 and id = $2 and checked = 
    false").await.unwrap();
    ...
    match result {
        ...
        _ => Err(io::Error::new(io::ErrorKind::Other, "Falied to ceheck list"))
    }
}

pub async fn delete_item(client: &Client, item_id: i32) -> Result<(), io::Error> {
    let statement = client.prepare("delete from todo_item where id = $1").await.unwrap();

    let result = client.execute(&statement, &[&item_id])
        .await
        .expect("Error checking todo item");
